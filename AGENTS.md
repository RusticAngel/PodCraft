...
- **IBM Bob**: `.opencode/agent/bob.md` is a `subagent` that drives BobShell (`bob -p "<task>"` / `--yolo`) as an SDLC partner. BobShell NOT installed yet — install with `powershell -ep Bypass 'irm -Uri "https://bob.ibm.com/download/bobshell.ps1" | iex'`, first run needs `bob --accept-license`, non-interactive needs API-key auth.
