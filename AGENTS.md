...
- **`call_with_retry` around every live API call** (TTS, Lyria, sentiment) — free-tier Gemini quota resets per minute and 429s are hit regularly. **Hardened 2026-08-21**: `is_transient()` now also retries 5xx server errors (`INTERNAL`/`UNAVAILABLE`/`DEADLINE_EXCEEDED`, codes 500/502/503/504) + `ConnectionError`/`TimeoutError`; TTS uses a deeper budget (`retries=6, base_delay=8s, max_delay=90s`); `AudioProducerAgent._retry_failed_segments` runs a second-chance pass after the main loop (20s cooldown, one retry per failed segment) so a transient error mid-job no longer leaves a silent hole; output reports `failed_segments` count.
...
### Episode Studio UI match — Lovable reference (added 2026-08-29)
Mirrors the "Episode Studio" prototype (`github.com/RusticAngel/podcast-perfect`, TanStack; reference fetched as `refs/remotes/upstream/main`, HEAD `8ed2cab`). Deliverables per task: `src/vibe_mapper.py`, `src/voice_manager.py`, `src/streamlit_app.py`; commit "Match Lovable UI design" → origin → `gcloud run deploy podcraft-ui`.
- `src/vibe_mapper.py` — EXACT prototype selector data + design tokens: `DESIGN` (oklch verbatim from `styles.css` + display hex; `bg #0D0F19 / card #171A26 / panel #232633 / primary ember #FB9A32 / accent teal #00AFAF`), `GENRES` = the 7 "Pick the vibe" options (technology/business/true crime/health/comedy/education/news), `VIBES` (genres + emoji 💻💼🕵️💚😄🎓📰), `MOODS` (9 presets incl. "Auto (match the script)"), sliders 10–100/60 and −40..0/−18, `intensity_label()` (Barely there/Gentle/Present/Bold). Vibe IS the backend genre (`genre_for()`).
- `src/voice_manager.py` — 6 Gemini voices (Puck/Charon/Kore/Fenrir/Aoede/Zephyr) metadata/emojis + `voice_labels/voice_by_label/recommend_for_role/voice_tag`.
- API: `GET /vibes` + `GET /voices`; `/upload` + `/jobs/upload` accept `music_mood` (auto/calm/happy/excited/serious/nervous/sad/funny/neutral), `music_intensity` (0.05–1.0 Query `ge`) and `duck_db` (−40..0) → threaded via `_process_pipeline` → `process_script` → `AudioProducerAgent.run/_produce_audio` → `LyriaMusicTool.generate_music(mood, 30s, intensity)`; output adds `music_config {mood,intensity,duck_db}`. Video beds honor ducking: `_run_video_job` reads `manifest.music_config.duck_db` → `10**(db/20)` (e.g. −18 dB→0.126) → `generate_video_from_pack(music_volume=)`.
- `audio_utils.synth_placeholder_wav` gained `intensity` (note_amp = `0.045*(0.55+0.45*intensity)`; seed+filename embed intensity → `music_{mood}_{dur}s_i{int}_placeholder.wav`). `lyria_music` added `_intensity_phrase`.
- **Lyria retries=1 (important)**: Lyria is paid-only, so on free tier it 429s and previously burned ~100s in `call_with_retry` (4 retries) before falling back to placeholder — blew the 60s job test + made demos crawl. `_generate_lyria` now uses `retries=1` (one fast attempt → instant placeholder fallback). Everything else keeps deep retry budgets.
- `static/demo_script.txt` = demo copy of `tests/Latest.txt` (narrator-role script).
- Streamlit (`src/streamlit_app.py` + `.streamlit/config.toml` primaryColor `#FB9A32`): "deep ink + warm ember" clone via `DESIGN` oklch CSS (radial stage glow, studio card, ember CTA, clip items). Mirror layout: hero, Step 1 dropzone (PDF/TXT/MD/DOCX + 2 demo chips), Step 2 vibe select, Step 3 Music & mix (mood + hint, intensity & duck sliders), advanced expander (lite segments + "Detect speakers & choose voices" → per-speaker voice selects → `voice_overrides`), Produce CTA → `_poll_job` animates the 5 stage progress, results (episode card w/ pack download + speaker badges + voice clips + st.audio, music bed panel, recommendations, video gen → MP4/MP3/SRT, JSON). Verified via `streamlit.testing.v1.AppTest` (0 exceptions; defaults airplane: vibe Technology, mood Auto, 60/−18).
...
- **Prominent voice casting (Step 3)**: Per-speaker voice dropdowns moved from the Advanced expander to a visible Step 3 "Cast the voices" in the main form. Uses `recommend_for_role()` for smart defaults. Voice overrides flow to `/jobs/upload` as before.
- **Video toggle**: `render_video` + `video_captions` boolean toggles added to the form (Step 4 "Music & mix"). These are passed as query params to `/jobs/upload`.
- **Backend**: `POST /upload` and `POST /jobs/upload` accept `render_video` (default True) and `video_captions` (default True). `_process_pipeline` generates video after audio if `render_video` is True, appends to pack, includes `video_url` in response.
- **Results**: Video panel shows inline if generated during produce (no separate "Generate video" button needed). Fallback "Generate video" button still available if video wasn't generated.
- **78 tests pass**.
...
## Current git state
- Branch `main`. Next commit: **"Match Lovable UI design"** (verified local stack on 8001/8501; 71 tests pass). Prior commits: `06eda8f` (Speaker-N labels, 60 tests), `e3c0636` (AGENTS.md key/billing state), `2222d15` (check_key.py), `3b8381d` (harden API), `b390f40` (v2 UI/video).
