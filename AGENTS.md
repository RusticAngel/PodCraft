...
- **Narrator role fix (2026-08-21)**: `_infer_role` now strips parenthesized labels (e.g. `OUTRO (Narrator)` → matches on `outro`) and `outro`/`intro` are `support` role hints → OUTRO is no longer mis-inferred as host. 45 tests pass.
